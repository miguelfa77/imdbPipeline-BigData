...
for file_name in file_names:
    ...
    for chunk in chunks:
        try:
            if first_chunk:
                chunk.to_sql(name=file_name, con=engine, if_exists='replace', index=False, chunksize=CHUNKSIZE)
                first_chunk = False
            else:
                chunk.to_sql(name=file_name, con=engine, if_exists='append', index=False, chunksize=CHUNKSIZE)
        except Exception as e:
